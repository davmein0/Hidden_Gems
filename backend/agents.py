...
User input: Ticker = {{$json.chatInput}}
...
Return a JSON string with only the management section completed.
"""
financials_prompt = """
...
def call_industry_agent(query: str):
    return industry_agent.query(f"Given {system_prompt} and {industry_prompt}, Analyze this ticker: {query}").response

def call_management_agent(query: str):
    return management_agent.query(f"Given {system_prompt} and {management_prompt}, Analyze this ticker: {query}").response

def call_product_agent(query: str):
    return product_agent.query(f"Given {system_prompt} and {product_prompt}, Analyze this ticker: {query}").response

def call_financials_agent(query: str):
    return financials_agent.query(f"Given {system_prompt} and {financials_prompt}, Analyze this ticker: {query}").response
...
1. Call the appropriate worker tools.
2. Wait for their JSON responses.
3. Merge them into a unified final JSON object.
4. NEVER invent anything that workers did not provide.
5. ALWAYS produce valid JSON.

Workers you MUST call:
- industry_agent
- management_agent
- product_agent
- financials_agent
